...
if ($cha_itm_loc -eq 'character') {
    ...
    #$cha_gender_spec=Read-Host -Prompt "Enter the gender of $cha_name (example: male, female, or genderless (robot))"
    ...
    Write-Host "$cha_name is a $cha_age $cha_gender $cha_species, and has $cha_color $cha_skin_fur. $cha_gender_spec is $cha_height in height, and $cha_weight in weight."
}
